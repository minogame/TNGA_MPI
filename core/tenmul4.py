    ...
    def __init__(self, name='no_name', from_variable=None, shape=[1, 2, 3, 4], trainable=True,
        ...
        if from_variable is not None:
            ...
            if isinstance(self.initializer, Tensor):
                ...
                self.tensor = tf.compat.v1.get_variable(name=self.name, shape=self.shape, initializer=self.initializer, trainable=trainable)
...
class TensorNetwork(object):
    def __init__(self, adj_matrix, name_list=None, initializer_list=None, trainable_list=None, scope='TensorNetwork'):
        ...
        if name_list is not None:
            ...
            self.name_list = list(RealTensor.letter_range(self.dim))
    ...
    def __add__(self, TN_b):
        return self.reduction() + TN_b.reduction()

    def __sub__(self, TN_b):
        return self.reduction() - TN_b.reduction()

    def __mul__(self, TN_b):
        return self.__tf_matmul__(self.reduction(), TN_b.reduction())
